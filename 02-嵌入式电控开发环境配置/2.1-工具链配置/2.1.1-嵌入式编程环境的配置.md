...
# 点击下载第一个.exe文件，选择文件路径E:\SPR\tool\tool-chain\MSYS2（自己按照上一章的要求进行创建文件夹），
 ![alt text](图库/image-8.png)
 # 注意弹出的终端应该是 UCRT64（黄色的终端） 而不是MSYS2（粉色的终端），如下图左边第一章黄色UCRT64，并且回到刚刚的MSYS2的官网，在这个终端中按照官网的5~8走完流程
 ![alt text](图库/image-44.png)
 ![alt text](图库/image-9.png)
 # 在完成第八步后，自己的电脑键盘 win+R，输入cmd，打开终端，输入gcc --version，如果出现版本号，则说明MSYS2安装成功,如下图
 ![alt text](图库/image-45.png)
...
### 安装 刚刚的终端中，注意终端的颜色（依旧是黄色，UCRT64） 输入 pacman -S mingw-w64-ucrt-x86_64-arm-none-eabi-toolchain 如图
![alt text](图库/image-10.png)
### 验证 arm-none-eabi-gcc --version 效果如下为有效
![alt text](图库/image-11.png)
保险起见，回到自己的电脑的终端（和上一步版本验证一样）输入arm-none-eabi-gcc --version，进行版本号的查询，
![alt text](图库/image-46.png)
## 最终环节环境变量
...
### 3.2 回到步骤二 点击用户变量（注意是系统变量还是用户变量，电脑不同上下可能会不一样，注意区分）下的PATH（严禁新建变量），点击编辑 如图 粘贴刚刚复制的文件工具链路径，
...
# 点击安装 F:\江协科技资料包\STM32入门教程资料\工具软件（这是我电脑的路径，D盘自行替换），安装驱动
![alt text](图库/image-47.png)
# 串口助手 即点即用 无需下载（在你自己的E:\SPR\tool\usart_assisant文件夹，把助手剪切过去，方方便查找和日常使用）
